...
function createTypographyElements<CustomProps extends Record<string, any>>(
  ...
  const Heading = withTypography(HeadingBasic, typographyParams, config);
  const Paragraph = withTypography(ParagraphBasic, typographyParams, config);
  const TextSection = withTypography(
    TextSectionBasic,
    typographyParams,
    config,
  );
